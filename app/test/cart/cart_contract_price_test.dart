...
class _StubPriceResolutionService implements PriceResolutionService {
  const _StubPriceResolutionService();
...
class _CartOrdersRepository extends FakeOrdersRepository {
  _CartOrdersRepository(this._lines);

  final List<CartLine> _lines;

  @override
  Future<List<CartLine>> fetchCartLines(String orderId) async => _lines;
}
...
void main() {
  ...
  testWidgets('cart line shows effective price and contract chip',
      ...
      ProviderScope(
        overrides: [
          ...
          ordersRepositoryProvider.overrideWithValue(
            _CartOrdersRepository(<CartLine>[line]),
          ),
          ...
          priceResolutionServiceProvider
              .overrideWithValue(const _StubPriceResolutionService()),
    ...
    expect(
      find.byKey(const ValueKey('cart_line_price_VAR-1')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey('cart_line_contract_chip_VAR-1')),
      findsOneWidget,
    );
  });
}
