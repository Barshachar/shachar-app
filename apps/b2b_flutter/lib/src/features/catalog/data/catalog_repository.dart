...
class SupabaseCatalogRepository implements CatalogRepository {
  ...
  Future<List<Product>> fetchProducts({bool refresh = false}) async {
    ...
    while (true) {
      final PagedProducts page = await fetchProductsPage(
        limit: pageSize,
        offset: offset,
        refresh: true,
      );
  ...
  Future<List<ProductSearchResult>> searchProducts({
    ...
    final Map<String, double?> priceCache = <String, double?>{};
    ...
    for (final Product product in products) {
      for (final ProductVariant variant in product.variants) {
        ...
        if (priceCache.containsKey(variant.id)) {
          unitPrice = priceCache[variant.id];
        } else {
          try {
            final effective = await pricingRepository.resolveEffectivePrice(
            ...
            unitPrice = effective.unitPrice;
            priceCache[variant.id] = unitPrice;
          } catch (_) {
            priceCache[variant.id] = null;
