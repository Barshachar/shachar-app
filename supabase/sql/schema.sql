...
-- Allow the canonical DATABASE_URL user (postgres) to assume supabase_admin for local QA sweeps.
...
create type cost_center_status as enum ('active','archived');
...
create table cost_centers (
    ...
    approver_name text,
    ytd_budget numeric(14,2) not null default 0,
    ytd_spent numeric(14,2) not null default 0,
...
create type payment_terms_code as enum ('net_30','net_45','net_60','due_on_receipt','two_ten_net_30');

create table payment_terms_templates (
    id uuid primary key default uuid_generate_v4(),
    code payment_terms_code not null,
...
create table vendor_payment_term_settings (
    vendor_id uuid primary key references companies(id) on delete cascade,
    default_terms_id uuid not null references payment_terms_templates(id),
    early_pay_discount_pct numeric(5,2),
    early_pay_discount_days int,
    late_fee_interest_pct numeric(5,2),
    grace_period_days int,
    allow_vendor_overrides boolean not null default false,
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now()
);

create table vendor_payment_term_options (
    id uuid primary key default uuid_generate_v4(),
    vendor_id uuid not null references companies(id) on delete cascade,
    terms_id uuid not null references payment_terms_templates(id),
    is_allowed boolean not null default true,
    unique(vendor_id, terms_id)
);

create table vendor_payment_term_overrides (
    id uuid primary key default uuid_generate_v4(),
    vendor_id uuid not null references companies(id) on delete cascade,
...
create type warehouse_bin_fill as enum ('empty','partial','full');
...
create table warehouse_bins (
    ...
    aisle text not null,
    bin text not null,
    fill_state warehouse_bin_fill not null default 'empty',
    current_qty numeric(14,2) not null default 0,
    capacity numeric(14,2),
    updated_at timestamptz not null default now(),
    unique(zone_id, aisle, bin)
);
...
create table customer_profiles (
    customer_id uuid primary key references companies(id) on delete cascade,
...
create table vendor_metrics (
    ...
    lead_time_days numeric(5,2) not null default 0,
    minimum_order_qty numeric(12,2) not null default 0,
...
grant usage on schema public to anon, authenticated, supabase_storage_admin;
grant select, insert, update, delete on all tables in schema public to anon, authenticated;
grant usage, select on all sequences in schema public to anon, authenticated;
grant execute on all functions in schema public to anon, authenticated;
grant execute on function auth_role() to supabase_storage_admin;
grant execute on function auth_company_id() to supabase_storage_admin;
