...
-- Storage policies for signed access
insert into storage.policies (id, bucket_id, name, definition, action, "created_at")
values
  (gen_random_uuid(), 'kyc_docs', 'Vendor KYC read own', '(((auth.jwt()->>''role'') = ''admin'') OR (auth.jwt()->>''company_id'' IS NOT NULL))', 'read', now()),
  (gen_random_uuid(), 'attachments', 'Attachments read', 'true', 'read', now())
